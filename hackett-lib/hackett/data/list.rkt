...
(provide (data List) head last tail init head! last! tail! init! uncons uncons! unfoldr nil? length
         ...
         fold-map or and any? all? elem? not-elem? delete delete-by intersperse)
...
(def tails : (forall [a] {(List a) -> (List (List a))})
  (λ [l] {l :: (case l
                 [{x :: xs} (tails xs)]
                 [Nil       Nil])}))

(def inits : (forall [a] {(List a) -> (List (List a))})
  (λ [l] {Nil :: (case l
                   [Nil       Nil]
                   [{x :: xs} (map (:: x) (inits xs))])}))
...
(def fold-map : (forall [a m] (Monoid m) => {{a -> m} -> (List a) -> m})
  (λ [f] (foldr {++ . f} mempty)))
